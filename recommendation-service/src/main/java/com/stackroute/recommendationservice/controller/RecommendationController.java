...
public class RecommendationController {
	@Autowired
	private RecommendationServiceApplication recommendationbookservice;
	...
	public @ResponseBody ResponseEntity<List<RecommendationBook>> getMyBooks(ServletRequest req, ServletResponse res) {
		...
		String token = authHeader.substring(7);
		String userId = Jwts.parser().setSigningKey("secretkey").parseClaimsJws(token).getBody().getSubject();
		return new ResponseEntity<List<RecommendationBook>>(recommendationbookservice.getMyRecommendationBooks(userId),
				HttpStatus.OK);
	}
